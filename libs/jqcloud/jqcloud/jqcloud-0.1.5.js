...
(function( $ ){
  $.fn.jQCloud = function(word_array, callback_function) {
    ...
    var drawWordCloud = function() {
      ...
      var min_top = 0;//document.getElementById($this.attr('id')).offsetTop;
      var max_bottom = document.getElementById($this.attr('id')).offsetHeight;//$this.height() - min_top;
      
      
      var min_left = 2;
      var max_right = $this.width() - min_left;
      ...
      $.each(word_array, function(index, word) {
        ...
        var word_right = document.getElementById(word_id).offsetLeft + min_left + document.getElementById(word_id).offsetWidth;
        
        while(hitTest(document.getElementById(word_id), already_placed_words) 
              || document.getElementById(word_id).offsetLeft < min_left 
              || word_right > max_right
              || top < min_top
              || top + height > max_bottom) {
                                             
            if((document.getElementById(word_id).offsetLeft < min_left || word_right > max_right)
               && (top < min_top || top + height > max_bottom))
            {
                $(word_selector).remove();
                return;
            }
            radius += step;
            ...
            $(word_selector).css('left', left + "px");
            $(word_selector).css('top', top + "px");
            word_right = document.getElementById(word_id).offsetLeft + min_left + document.getElementById(word_id).offsetWidth;
        }
